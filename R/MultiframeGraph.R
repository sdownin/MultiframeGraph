...
#' multiContractedStar plots
#' 
#' Multiframe star plot of igraph networks contracted by given attribute
#' (best for more vertices)
#' 
...
#' the subgraph nodes.
#' @details 
#' This function creates a multiframe star layout plot of subgraphs from a list of 
#' igraph objects with vertices labeled by ID number. Each graph in the 
#' list is a row and each time period is a column. Each igraph object in the 
#' list must have the same vertex attribute names. 
#' Each igraph object in the list must have the time period vertex
#' attribute called "period", a numeric or interger attribute. 
#' Vertex coloring is automatically generated from choice of 
#' vertex.color.attrib argument. 
#' @author Stephen Downing
#' @import igraph
#' @seealso
...
multiContractedStar <- function(graphlist,
                            attrib,
                            pdbreak,
...
#' multiContractedCircle plots
#' 
#' Multiframe circle plot of igraph networks contracted by given attribute
#' (best for fewer vertices)
#' 
#' @param graphlist  a list of igraph objects (i.e., graphs)
#' @param attrib  character name of igraph vector attribute for coloring, sizing
#' @param pdbreak a vector of the numeric values to divide periods
...
#' the subgraph nodes.
#' @details 
#' This function creates a multiframe circle layout plot of subgraphs from a list
#' of igraph objects with vertices labeled by attribute name. Each graph in the 
#' list is a row and each time period is a column. Each igraph object in the 
#' list must have the same vertex attribute names. 
#' Each igraph object in the list must have the time period vertex
#' attribute called "period", a numeric or interger attribute. 
...
multiContractedCircle <- function(graphlist,
                                attrib,
                                pdbreak,
                                vertex.size.attrib='degree', 
                                #{'degree','indeg','outdeg','degdiff'}
  ...
  for ( i in seq_len(mn)) {
    ...
    V(sub)$vertexlabelcolor <- V(sub)$color
    ...
    if (vcount(sub) > 0) {
      ...
      plot.igraph(sub,
                  ...
                  vertex.label=V(sub)$id,
                  vertex.label.cex=V(sub)$vertexlabelcex,
                  vertex.label.color=V(sub)$vertexlabelcolor,
                  vertex.color=V(sub)$color,
                  vertex.label.font=7,
                  edge.arrow.size=E(sub)$edgearrowsize,
                  edge.width=E(sub)$edgewidth,
